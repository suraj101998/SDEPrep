...
/*
 * Question: Service Request Assignment System
A company provides on-site services in different cities. Customers raise service requests, and each request needs to be assigned to a salesperson in the same city. The company follows these rules for assignment:

Rules for Assignment:
Each service request has an ID, priority (High, Medium, Low), location, and status (Pending, Assigned, or Completed).
Each salesperson has an ID, location, and a maximum number of requests (capacity) they can handle at a time.
...
Task:
Write a Java program to assign service requests to salespersons based on the above rules and print the final assignments.

Constraints:
1≤𝑁,𝑀≤10^3
1≤N,M≤10^3
 
Priorities are only "High", "Medium", or "Low".
Locations are case-sensitive (CityA is different from citya).
All Salesperson_ID and Request_ID are unique.
No two requests have the same ID.
 ...
 */
 ...
 public class RequestAssignmentSystem {
     public static void main(String[] args) {
         ...
         List<ServiceRequest> requests = Arrays.asList(
             ...
             new ServiceRequest(102, "Medium", "CityA", "Pending"),
             new ServiceRequest(103, "Low", "CityB", "Pending"),
             new ServiceRequest(104, "High", "CityB", "Pending"),
             new ServiceRequest(105, "Medium", "CityA", "Pending"),
             new ServiceRequest(106, "Medium", "CityA", "Pending")
         );
...
/*
 * Time Complexity:
Sorting Requests: O(N log N), where N = number of requests.
Assigning Requests: O(N * M), where M = number of salespersons.
Overall Complexity: O(N log N + N * M)

Space Complexity:
Storing salespersons and requests in memory.
