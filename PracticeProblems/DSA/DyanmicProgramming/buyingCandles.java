package PracticeProblems.DSA.DyanmicProgramming;
/*
 * Problem Description
...
public class buyingCandles {
    public int solve(int[] A, int[] B, int[] C, int D) {
        ...
        // Iterate over all candy packets
        for (int i = 0; i < A.length; i++) {
            int sweetness = A[i] * B[i]; // Total sweetness of the i-th packet
            int cost = C[i];            // Cost of the i-th packet
            
            // Update DP array for each nibble count
            for (int j = cost; j <= D; j++) {
                dp[j] = Math.max(dp[j], dp[j - cost] + sweetness);
            }
        }
